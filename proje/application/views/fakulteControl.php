	...
	<div class="content-wrapper">
		...
		<section class="content">
			...
			<div class="container box">
				<h3 align="center">Fakülte ve Bölümleri Düzenle</h3><br />
				<div class="table-responsive">
					<br />
					<table class="table table-striped table-bordered">
						<thead>
						<tr>
							<th style="text-align: center">Fakülte</th>
							<th style="text-align: center">Bölüm</th>
							<th style="text-align: center">Bölüm Tür</th>
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function(){
		...
		{
			$.ajax({
				url:"<?php echo base_url(); ?>home/load_data",
				dataType:"JSON",
				success:function(data){
					var html = '<tr>';
					html += '<td id="fakulte" contenteditable placeholder="Fakülte Adı boş olamaz "></td>';
					html += '<td id="bolum" contenteditable placeholder="Bölüm boş olamaz"></td>';
					html += '<td id="bolum_tur" contenteditable></td>';
					...
					{
						html += '<tr>';
						html += '<td class="table_data" data-row_id="'+data[count].id+'" data-column_name="fakulte" contenteditable>'+data[count].fakulte+'</td>';
						html += '<td class="table_data" data-row_id="'+data[count].id+'" data-column_name="bolum" contenteditable>'+data[count].bolum+'</td>';
						html += '<td class="table_data" data-row_id="'+data[count].id+'" data-column_name="bolum_tur" contenteditable>'+data[count].bolum_tur+'</td>';
		...
		$(document).on('click', '#btn_add', function(){
			var fakulte = $('#fakulte').text();
			var bolum = $('#bolum').text();
			var bolum_tur = $('#bolum_tur').text();
			if(fakulte == '')
			{
				alert('Fakülte Adı boş Bırakılmaz.');
				return false;
			}
			if(bolum == '')
			{
			...
			$.ajax({
				...
				method:"POST",
				data:{fakulte:fakulte, bolum:bolum, bolum_tur:bolum_tur},
				success:function(data){
					load_data();
				}
			})
